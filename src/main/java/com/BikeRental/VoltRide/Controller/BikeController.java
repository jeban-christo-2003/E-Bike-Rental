...
public class BikeController {
    ...
    public ResponseEntity<Map<String, Object>> login(@RequestBody Model modelObj) {
        try {
            ...
            if (ok) {
                return ResponseEntity.ok(Map.of("success", true, "message", "BikeRental authenticated successfully"));
            } else {
                return ResponseEntity.status(HttpStatus.UNAUTHORIZED)
                        .body(Map.of("success", false, "message", "BikeRental authentication failed"));
            }
        } catch (Exception e) {
    ...
    public List<Vehicle> vehicleList() {
        return List.of(new Vehicle(1, "E-Volt-Bike"), new Vehicle(2, "EcoRider"), new Vehicle(3, "VoltRacer"));
    }
}
